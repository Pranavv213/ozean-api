...
async function hasInteractedWithBridgeOnSepolia(walletAddress, startBlock = 0, endBlock = 'latest') {
    ...
    const tokenAddress = "0x15795aadca3759d7b356DecE036c285b1FBb32aa";


    // ERC-20 Token ABI (minimum required for transfer event)
    const tokenAbi = [
        {
          "inputs": [
            {
...
const filter = tokenContract.filters.Transfer(walletAddress, contractAddress);

// Query logs for the `Transfer` event
...
logs.forEach(log => {
    ...
    console.log(`Amount: ${ethers.utils.formatUnits(args.value, 18)} tokens`);
